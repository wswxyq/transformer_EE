...
# %%
from transformer_ee.model.transformerEncoder import Transformer_EE_v2
...
for i in range(epochs):
    ...
    for (batch_idx, batch) in enumerate(validloader):
        vector_valid_batch = batch[0].to(mps_device)
        scalar_valid_batch = batch[1].to(mps_device)
        mask_valid_batch = batch[2].to(mps_device)
        target_valid_batch = batch[3].to(mps_device)
...
net=Transformer_EE_v2()
net.load_state_dict(torch.load("transformer_ee/model/transformer_ee_v2.pt", map_location=torch.device('cpu')))
net.eval()
